...
import {Claim} from "src/libs/utils/Structs.sol";
...
library HydraS2Lib {
  ...
  function _encodeRequestIdentifier(
    ...
    bytes32 groupSnapshotId = _encodeGroupSnapshotId(self.groupId, self.groupTimestamp);
  ...
  function _encodeAccountsTreeValue(Claim memory self) internal pure returns (uint256) {
    return uint256(_encodeGroupSnapshotId(self.groupId, self.groupTimestamp)) % SNARK_FIELD;
  }
  ...
  function _encodeServiceId(bytes16 appId, bytes16 namespace) private pure returns (bytes32) {
