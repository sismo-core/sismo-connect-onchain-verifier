...
import {IHydraS2Verifier, ZkConnectProof, VerifiedAuth, VerifiedClaim} from "./IHydraS2Verifier.sol";
import {Claim, Auth, ClaimType, AuthType} from "../libs/utils/Structs.sol";
...
contract HydraS2Verifier is IHydraS2Verifier, IBaseVerifier, HydraS2SnarkVerifier, Initializable {
  ...
  function _verifyClaimValidity(
    ...
    uint256 expectedRequestIdentifier = claim._encodeRequestIdentifier(appId, namespace);
    ...
    // accountsTreeValue
    uint256 groupSnapshotId = claim._encodeAccountsTreeValue();
    ...
    if (claimComparatorEQ != isClaimTypeFromClaimEqualToEQ) {
      revert ClaimTypeMismatch(input.claimComparator, claim.claimType);
    }
