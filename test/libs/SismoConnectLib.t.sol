...
contract SismoConnectLibTest is VerifierMockBaseTest {
  ...
  function test_RevertWith_InvalidUserIdAndAuthType() public {
    ...
    sismoConnect.exposed_buildAuth({authType: AuthType.VAULT, isOptional: false, isSelectableByUser: false, userId: uint256(bytes32("wrong-id"))});
  }

  function test_RevertWith_VersionMismatch() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withVersion(bytes32("wrong-version")).build();
  ...
  function test_RevertWith_NamespaceMismatch() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withNamespace(bytes16(keccak256("wrong-namespace"))).build();
  ...
  function test_RevertWith_SignatureMessageMismatch() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withSignedMessage("wrong-signed-message").build();
  ...
  function test_RevertWith_AuthTypeAndUserIdNotFound() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withAuth({auth: AuthBuilder.build({authType: AuthType.VAULT, userId: uint256(bytes32("wrong-id"))})});
    // we need to choose a different AuthType than AUthType.VAULT to be able to test if the userId error is thrown
    // we set userId of the request to 0xf00 to be different from the one in the response
    AuthRequest memory auth = sismoConnect.exposed_buildAuth({authType: AuthType.GITHUB, userId: uint256(0xf00)}); // wrong userId
  ...
  function test_RevertWith_AuthUserIdNotFound() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withAuth({auth: AuthBuilder.build({authType: AuthType.GITHUB, userId: uint256(bytes32("wrong-id"))})});
    // we need to choose a different AuthType than AUthType.VAULT to be able to test if the userId error is thrown
    // we set userId of the request to 0xf00 to be different from the one in the response
    AuthRequest memory auth = sismoConnect.exposed_buildAuth({authType: AuthType.GITHUB, userId: uint256(0xf00)}); // wrong userId
  ...
  function test_RevertWith_ClaimGroupIdAndGroupTimestampNotFound() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withClaim({claim: ClaimBuilder.build({groupId: "wrong-id", groupTimestamp: bytes16("fake-timestamp")})});
    vm.expectRevert(
  ...
  function test_RevertWith_ClaimGroupTimestampNotFound() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withClaim({claim: ClaimBuilder.build({groupId: claimRequest.groupId, groupTimestamp: bytes16("wrong-timestamp")})});
  ...
  function test_RevertWith_ClaimTypeAndGroupIdNotFound() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withClaim({claim: ClaimBuilder.build({groupId: "wrong-id", claimType: ClaimType.LTE})});
    vm.expectRevert(
  ...
  function test_RevertWith_ClaimGroupIdNotFound() public {
    SismoConnectResponse memory invalidResponse = DEFAULT_RESPONSE.withClaim({claim: ClaimBuilder.build({groupId: bytes16("wrong-group-id")})});
