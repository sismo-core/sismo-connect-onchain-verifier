...
library ResponseBuilder {
  ...
  function withAppId(SismoConnectResponse memory response, bytes16 appId) external pure returns (SismoConnectResponse memory) {
    response.appId = appId;
    return response;
  }

  function withAppId(ResponseWithoutProofs memory response, bytes16 appId) external pure returns (ResponseWithoutProofs memory) {
    response.appId = appId;
    return response;
  }

  function withVersion(SismoConnectResponse memory response, bytes32 version) external pure returns (SismoConnectResponse memory) {
    response.version = version;
    return response;
  }

  function withVersion(ResponseWithoutProofs memory response, bytes32 version) external pure returns (ResponseWithoutProofs memory) {
    response.version = version;
    return response;
  }

  function withNamespace(SismoConnectResponse memory response, bytes16 namespace) external pure returns (SismoConnectResponse memory) {
    response.namespace = namespace;
    return response;
  }

  function withNamespace(ResponseWithoutProofs memory response, bytes16 namespace) external pure returns (ResponseWithoutProofs memory) {
    response.namespace = namespace;
    return response;
  }

  function withSignedMessage(SismoConnectResponse memory response, bytes memory signedMessage) external pure returns (SismoConnectResponse memory) {
    response.signedMessage = signedMessage;
    return response;
  }

  function withSignedMessage(ResponseWithoutProofs memory response, bytes memory signedMessage) external pure returns (ResponseWithoutProofs memory) {
    response.signedMessage = signedMessage;
    return response;
  }

  function withClaim(SismoConnectResponse memory response, Claim memory claim, bytes memory proofData) external pure returns (SismoConnectResponse memory) {
  ...
  function build(ResponseWithoutProofs memory response) public pure returns (SismoConnectResponse memory) {
